...
const fpal = {}; for (const f of ['celestial', 'spiritual', 'earth', 'treasure', 'zombie']) fpal[f] = S.palette('blue', false, f);
console.log(JSON.stringify({ frames: S.FRAMES, pal, food: F.ART, foodPal: F.PAL, kinds: F.KINDS, badges: B.LIST, badgeArt: B.ART, xl: X.FORMS, fpal }));
"""
...
def main():
    ...
    sheet([(data["xl"][f]["walk"][2], data["fpal"][f]) for f in ("celestial", "spiritual", "earth", "treasure", "zombie")], 6, cols=2).save(OUT / "final-forms.png")
